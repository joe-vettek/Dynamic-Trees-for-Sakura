...
public class UmeLogic implements IGrowthLogicKit {
     ...
     */
    public UmeLogic(float energyDivisor) {
     ...
     */
    public UmeLogic setHorizontalLimiter(float energyLimiter) {
     ...
     * @return ConiferLogic for chaining
     */
    public UmeLogic setHeightVariation(int heightVariation) {
    ...
    public int[] directionManipulation(World world, BlockPos pos, Species species, int radius, GrowSignal signal, int[] probMap) {
        ...
        probMap[1] = signal.isInTrunk()?species.getUpProbability() :species.getUpProbability() *3;
        probMap[2] = probMap[3] = probMap[4] = probMap[5] = //Only allow turns when we aren't in the trunk(or the branch is not a twig and step is odd)
                !signal.isInTrunk() || (signal.isInTrunk() && signal.numSteps % 2 == 1 && radius > 1) ? 2 : 1;
